...
log "Creating agent: Novel Editor..."
LEAD_RESP=$(api POST /v1/agents -d "$(cat <<'JSON'
{
  "agent_key": "novel-editor",
...
LEAD_ID=$(get_agent_id "$LEAD_RESP" "novel-editor")
info "Lead ID: ${LEAD_ID:-unknown}"

# --- Agent 2: Viết Lại (Member) ---
log "Creating agent: Novel Rewriter..."
REWRITER_RESP=$(api POST /v1/agents -d "$(cat <<'JSON'
{
  "agent_key": "novel-rewriter",
  "display_name": "Novel Rewriter",
  "agent_type": "predefined",
  "tools_config": {
    "enabled": [
...
REWRITER_ID=$(get_agent_id "$REWRITER_RESP" "novel-rewriter")
info "Rewriter ID: ${REWRITER_ID:-unknown}"

# --- Agent 3: Kiểm Duyệt (Reviewer) ---
log "Creating agent: Novel Reviewer..."
REVIEWER_RESP=$(api POST /v1/agents -d "$(cat <<'JSON'
{
  "agent_key": "novel-reviewer",
  "display_name": "Novel Reviewer",
  "agent_type": "predefined",
  "tools_config": {
    "enabled": [
      "read_file", "write_file", "edit",
...
REVIEWER_ID=$(get_agent_id "$REVIEWER_RESP" "novel-reviewer")
...
log "Setting SOUL.md for Novel Editor..."
set_agent_file "novel-editor" "SOUL.md" "$(cat <<'SOUL'
# Biên Tập Dịch
...
- Phân task cho Novel Rewriter (sửa văn) và Novel Reviewer (review)
...
4. Tạo task "Viết lại {section}" → assign "novel-rewriter"
5. Tạo task "Kiểm duyệt {section}" → assign "novel-reviewer", blocked_by task viết lại
...
log "Setting SOUL.md for Novel Rewriter..."
set_agent_file "novel-rewriter" "SOUL.md" "$(cat <<'SOUL'
# Viết Lại Văn Dịch
...
log "Setting SOUL.md for Novel Reviewer..."
set_agent_file "novel-reviewer" "SOUL.md" "$(cat <<'SOUL'
# Kiểm Duyệt Văn Dịch
...
log "Setting AGENTS.md for Novel Editor..."
set_agent_file "novel-editor" "AGENTS.md" "$(cat <<'AGENTS'
# Team Members

## novel-rewriter
- Nhà văn chuyên viết lại văn dịch thô thành tiếng Việt tự nhiên
- Input: văn bản dịch máy. Output: văn xuôi mượt mà
- Giao việc: `team_tasks(action="create", subject="Viết lại ...", assignee="novel-rewriter")`
...
- Giao việc: `team_tasks(action="create", subject="Kiểm duyệt ...", assignee="novel-reviewer")`
...
TEAM_RESP=$(api POST /v1/teams -d "$(cat <<'JSON'
{
  ...
  "lead": "novel-editor",
  "members": ["novel-rewriter", "novel-reviewer"],
  "description": "Fix badly translated Vietnamese novels: Editor assigns → Rewriter fixes prose → Reviewer QAs",
  "settings": {
    "version": 1,
...
echo ""
echo "  Agents:"
echo "    novel-editor       ${LEAD_ID:-N/A}"
echo "    novel-rewriter      ${REWRITER_ID:-N/A}"
echo "    novel-reviewer     ${REVIEWER_ID:-N/A}"
echo ""
echo "  Skills:"
...
echo ""
echo "  Cách dùng:"
echo "    1. Open Web UI → chat with 'Novel Editor'"
echo "    2. Paste đoạn truyện dịch thô, nói: 'Sửa lại đoạn này cho mượt'"
echo "    3. Pipeline: Editor → Rewriter → Reviewer → Done"
echo ""
