...
#   - websocat installed (for team creation via WebSocket RPC)
...
WS_URL="${GOCLAW_WS_URL:-ws://localhost:18790/ws}"
...
# ═══════════════════════════════════════════════
#  STEP 5: Create Team (WebSocket RPC)
# ═══════════════════════════════════════════════
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
log "Creating team via WebSocket RPC..."
TEAM_RESP=$(ws_rpc "teams.create" "$TEAM_PARAMS" 2>/dev/null || true)

if [[ -n "$TEAM_RESP" ]]; then
  TEAM_ID=$(echo "$TEAM_RESP" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
  info "Team ID: ${TEAM_ID:-check response}"
  if echo "$TEAM_RESP" | grep -q '"ok":true'; then
    log "Team created successfully!"
  else
    warn "Team response: $TEAM_RESP"
  fi
else
  warn "WebSocket RPC unavailable. Create team manually via Web UI:"
  echo ""
  echo "  Team name:    Translation Team"
  echo "  Lead:         translation-manager"
  echo "  Members:      translator, translation-reviewer"
